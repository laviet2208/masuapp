...
import 'package:masuapp/SCREEN/SHIPPERSCREEN/INUSER/PAGE_HOME/Page%20%C4%91%E1%BB%93%20%C4%83n/Chi%20ti%E1%BA%BFt%20%C4%91%C6%A1n%20%C4%91%E1%BB%93%20%C4%83n.dart';
import 'package:masuapp/SCREEN/SHIPPERSCREEN/INUSER/PAGE_HOME/Page%20%C4%91%E1%BB%93%20%C4%83n/Item%20%C4%91%E1%BB%93%20%C4%83n.dart';
...
class Pagedoan extends StatefulWidget {
  final double height;
  final double width;
  final Time data;
  final VoidCallback restartEvent;
  const Pagedoan({Key? key, required this.height, required this.width, required this.data, required this.restartEvent}) : super(key: key);

  @override
  State<Pagedoan> createState() => _PagedoanState();
}

class _PagedoanState extends State<Pagedoan> {
          ...
          itemBuilder: (context, index) {
            return GestureDetector(
              child: Padding(
                ...
                child: Itemdoan(width: widget.width, order: orderList[index]),
              ),
              onTap: () {
                Navigator.push(context, MaterialPageRoute(builder:(context) => Chitietdondoan(id: orderList[index].id, diemdon: orderList[index].locationSet, diemtra: orderList[index].locationGet)));
              },
            );
          }),
    );
