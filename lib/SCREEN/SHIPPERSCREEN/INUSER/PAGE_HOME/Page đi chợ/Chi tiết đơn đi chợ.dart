...
import 'package:masuapp/SCREEN/INUSER/PAGE_HOME/T%C3%ADnh%20kho%E1%BA%A3ng%20c%C3%A1ch.dart';
...
class Chitietdondicho extends StatefulWidget {
  final String id;
  final accountLocation diemdon;
  final accountLocation diemtra;
  const Chitietdondicho({Key? key, required this.id, required this.diemdon, required this.diemtra}) : super(key: key);

  @override
  State<Chitietdondicho> createState() => _ChitietdondoanState();
}

class _ChitietdondoanState extends State<Chitietdondicho> {
  ...
  Future<void> changeShipper(accountNormal ship) async {
    try {
      DatabaseReference databaseRef = FirebaseDatabase.instance.reference();
      await databaseRef.child('Order/productOrder/' + thiscatch.id + '/shipper').set(ship.toJson());
    } catch (error) {
      print('Đã xảy ra lỗi khi đẩy catchOrder: $error');
      throw error;
    }
  ...
  Future<void> changeStatus(String sta, String time) async {
    try {
      DatabaseReference databaseRef = FirebaseDatabase.instance.reference();
      await databaseRef.child('Order/productOrder/' + thiscatch.id + '/status').set(sta);
      databaseRef = FirebaseDatabase.instance.reference();
      await databaseRef.child('Order/productOrder/' + thiscatch.id + '/' + time).set(getCurrentTime().toJson());
    } catch (error) {
      print('Đã xảy ra lỗi khi đẩy catchOrder: $error');
      throw error;
    }
  ...
  Widget build(BuildContext context) {
    ...
    if (thiscatch.status == "C") {
      status = "Hãy tới cửa hàng lấy đồ ăn";
                          ...
                          if (thiscatch.status == 'C') {
                            ...
                            if (((thiscatch.cost * (thiscatch.costFee.discount/100)) + ((thiscatch.shipcost) * (thiscatch.costBiker.discount/100))) <= currentAccount.totalMoney) {
                              if (thiscatch.status == 'B') {
                                ...
                                historyTransaction his1 = historyTransaction(id: generateID(10), senderId: '', receiverId: currentAccount.id, transactionTime: getCurrentTime(), type: 5, content: 'Chiết khấu đơn ' + thiscatch.id, money: ((thiscatch.cost * (thiscatch.costFee.discount/100)) + ((thiscatch.shipcost) * (thiscatch.costBiker.discount/100))), area: currentAccount.Area);
