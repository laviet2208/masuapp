...
import 'package:masuapp/SCREEN/RESTAURANT/SCREENshopview.dart';
...
class _ITEMrestaurantIndirectState extends State<ITEMrestaurantIndirect> {
  ...
  void getData() {
    ...
    reference.child("Restaurant/" + widget.id).onValue.listen((event) {
    ...
    return GestureDetector(
      ...
      onTap: () {
        if (selectShop.OpenStatus == 0) {
          toastMessage('Nhà hàng đang đóng cửa');
        } else {
          ...
          if (isCurrentTimeInRange(openTime, closeTime)) {
            Navigator.push(context, MaterialPageRoute(builder:(context) => SCREENshopview(currentShop: widget.id,)));
          } else {
            toastMessage('Nhà hàng đóng cửa');
          }
        }
      },
    );
