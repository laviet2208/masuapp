...
class shipper_dashboard_day extends StatefulWidget {
  const shipper_dashboard_day({super.key});

  @override
  State<shipper_dashboard_day> createState() => _shipper_dashboard_dayState();
}

class _shipper_dashboard_dayState extends State<shipper_dashboard_day> {
    ...
    await reference.child("Order").orderByChild('shipper/id').equalTo(finalData.shipper_account.id).once().then((DatabaseEvent event) {
      ...
      orders.forEach((key, value) {
        if (value['resCost'] != null) {
          ...
          if (order.timeList[1].day == DateTime.now().day && order.timeList[1].month == DateTime.now().month && order.timeList[1].year == DateTime.now().year) {
