...
import '../store_main_screen/store_main_screen.dart';

class item_store_in_directory extends StatefulWidget {
  final String shopId;
  const item_store_in_directory({super.key, required this.shopId});

  @override
  State<item_store_in_directory> createState() => _item_store_in_directoryState();
}

class _item_store_in_directoryState extends State<item_store_in_directory> {
    ...
    return GestureDetector(
      ...
      onTap: () {
        if (account.openStatus == 1 && check_open_time(account.openTime, account.closeTime)) {
          Navigator.pushReplacement(context, MaterialPageRoute(builder:(context) => store_view_screen(shopId: widget.shopId, beforeWidget: store_main_screen())));
        } else {
          toastMessage('Cửa hàng tạm đóng cửa');
        }
      },
    );
  }
